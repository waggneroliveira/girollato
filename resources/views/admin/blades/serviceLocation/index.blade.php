                        ...
                        <div class="page-title-box">
                            <div class="page-title-right">
                                <ol class="breadcrumb m-0">
                                    ...
                                    <li class="breadcrumb-item active">Sessão Onde atendemos</li>
                                </ol>
                            </div>
                            <h4 class="page-title">Sessão Onde atendemos</h4>
                        </div>
                    </div>
                </div>
                <!-- end row -->
                                ...
                                <div class="row mb-2">
                                    <div class="col-12 d-flex justify-end">
                                        <div class="col-12 d-flex justify-content-end">
                                            @if (Auth::user()->can('onde atendemos.visualizar') &&
                                            Auth::user()->can('onde atendemos.criar') ||
                                            Auth::user()->can('usuario.tornar usuario master') || 
                                            Auth::user()->hasRole('Super'))
                                                @if (!$serviceLocation)                                                
                                                    <button type="button" class="btn btn-primary text-black waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#serviceLocation-create"><i class="mdi mdi-plus-circle me-1"></i> {{__('dashboard.btn_create')}}</button>
                                                @endif
                                                <!-- Modal -->
                                                        ...
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body p-2 px-3 px-md-4">
                                                                <form action="{{ route('admin.dashboard.serviceLocation.store') }}" method="POST" enctype="multipart/form-data">
                                                                    @csrf
                                                                    @include('admin.blades.serviceLocation.form', ['textareaId' => 'textarea-create'])  
                                        ...
                                        <tbody>
                                            @if (isset($serviceLocation))                                                
                                                <tr>
                                                    ...
                                                    <td>{{$serviceLocation->title}}</td>
                                                    <td class="table-user">
                                                        @if ($serviceLocation->path_file)                                                            
                                                            <a href="{{ asset('storage/'.$serviceLocation->path_file) }}" target="_blank" rel="noopener noreferrer" download="arquivo">
                                                    ...
                                                    <td class="d-flex gap-lg-1 justify-center">
                                                        @if (Auth::user()->can('onde atendemos.visualizar') &&
                                                        Auth::user()->can('onde atendemos.editar') ||
                                                        Auth::user()->can('usuario.tornar usuario master') || 
                                                        Auth::user()->hasRole('Super'))
                                                            <button class="table-edit-button btn btn-primary text-black" data-bs-toggle="modal" data-bs-target="#modal-group-edit-{{$serviceLocation->id}}" style="padding: 2px 8px;width: 30px"><span class="mdi mdi-pencil"></span></button>
                                                        ...
                                                        @if (Auth::user()->can('onde atendemos.visualizar') &&
                                                        Auth::user()->can('onde atendemos.remover') ||
                                                        Auth::user()->can('usuario.tornar usuario master') || 
                                                        Auth::user()->hasRole('Super'))
